...
var ValidateProperties = {
	...
	AjaxU: "fv-ajaxu",//ajax验证username
	AjaxE: "fv-ajaxe",//ajax验证email
...
ElementValidator.prototype.Validate = function () {
	return  this.CheckEmpty() && this.CheckDataType() && this.CheckFormat() && this.CheckMaxLength() && this.CheckMinLength() && this.CheckMaxValue() && this.CheckMinValue() && this.CheckCompareTo() && this.CheckAjaxUserName() && this.CheckAjaxEmail() && this.CheckCustom();
};
//验证值是否为空
...
//验证Ajax
function chkAjax(id1,id2,url,msg){
	var chk = true;
	$.ajax({
		type: 'POST',
		dataType:'json',
		...
		url: url,
		error: function(callBackMsg) {
			return null;
		},
		success: function(callBackMsg){ 
		  if(callBackMsg.flag == 0){
			this.SetValidateStatus(chk, $("#"+id2).html(msg+"可用"));
			// $("#"+id2).html(msg+"可用");
		  }else{
			chk = false;
			this.SetValidateStatus(chk, $("#"+id2).html(msg+"已存在"));
			// $("#"+id2).html(msg+"已存在");
...
ElementValidator.prototype.CheckAjaxUserName = function () {
var chk = true;
var msg = null;
if (!DataType.String.IsNullOrEmpty(this.Body.value)) {
var config = this.GetValidateConfig(ValidateProperties.AjaxU);
		if (config != null) {
			// chk = GetLength(this.Body) <= parseInt(config.Value);
			$.ajax({
				type: 'POST',
				...
				url: "/racommon/checkUserName/",
				error: function(callBackMsg) {
					return null;
				},
				success: function(callBackMsg){ 
				if(callBackMsg.flag == 0){
					var e1 = document.getElementById("msgnc"); 
					removeClass(e1, ValidateCssClass.Error);
					removeClass(e1, ValidateCssClass.Info);
					className = ValidateCssClass.Succeed;
					addClass(e1, className);
				if (e1 != null) {
					if (e1.getAttribute(ValidateProperties.MsgInfo) == null) {
					//保存默认消息，用于验证后,没有设置验证失败消息，也没有设置fv-message的情况。
					e1.setAttribute(ValidateProperties.MsgInfo, e1.innerHTML);
					}
					if (DataType.String.IsNullOrWhiteSpace(msg)) {
					msg = e1.getAttribute(ValidateProperties.MsgInfo);
					}
					e1.innerHTML = msg;
					...
					chk = false; 
					var msgpanel = document.getElementById("msgnc");   
					removeClass(msgpanel, ValidateCssClass.Succeed);
					removeClass(msgpanel, ValidateCssClass.Info);
					className = ValidateCssClass.Error;
					msg = config.Message;
				if(DataType.String.IsNullOrEmpty(msg)) {
					msg = msgpanel.getAttribute(ValidateProperties.MsgError);
				}
				addClass(msgpanel, className);
				if (msgpanel != null) {
					if (msgpanel.getAttribute(ValidateProperties.MsgInfo) == null) {
					//保存默认消息，用于验证后,没有设置验证失败消息，也没有设置fv-message的情况。
						msgpanel.setAttribute(ValidateProperties.MsgInfo, msgpanel.innerHTML);
...
ElementValidator.prototype.CheckAjaxEmail = function () {
var chk = true;
var msg = null;
if (!DataType.String.IsNullOrEmpty(this.Body.value)) {
var config = this.GetValidateConfig(ValidateProperties.AjaxE);
		if (config != null) {
			// chk = GetLength(this.Body) <= parseInt(config.Value);
			$.ajax({
				type: 'POST',
				...
				url: "/racommon/checkEmail/",
				error: function(callBackMsg) {
					return null;
				},
				success: function(callBackMsg){ 
				if(callBackMsg.flag == 0){
					var e1 = document.getElementById("msgpanel"); 
					removeClass(e1, ValidateCssClass.Error);
					removeClass(e1, ValidateCssClass.Info);
					className = ValidateCssClass.Succeed;
					addClass(e1, className);
